...
export default function ZoneBar({ zones, now, baseZoneId, onSetBase, onRemove }: Props) {
  const canRemove = zones.length > 1;
  return (
    <div className="zonebar">
      {zones.map((z) => {
        ...
        return (
          <div key={z.id} className="zonecard-wrap">
            <button
              type="button"
